...
## Why use bash?
Generally, what can be done in one programming language, can be done in any programming language. But somethings are easier to do in particular languages. Bash is easy to do simple things, but very complicated to do complicated things. That may seem obvious, but in some languages like python, doing complicated things can somethings just be one line of code. In this tutorial, we will go over some of the easier, basic functions of bash. Here are some examples of easy things that can be done in bash:

* Move, copy, and delete files
  * This is replacing your mouse, with the advantage being that you can manipulate lots of files at the same time instead of pointing and clicking a thousand times
* Download programs and packages
  * Think "Internet Explorer or Microsoft Edge". It's only useful for downloading Google Chrome
  * Bash can be used to install packages (mini-programs) that are used by python and R
* Run programs and scripts
  * When you create scripts in other languages (like python and R), these scripts can be run using bash, instead of opening a python intepretor, opening your file manually, and running the code. 
...
### Recommended set up
We recommend that you have the terminal, the tutorial notebook, and your text editor open and easily accessible on your computer. This will help you read directions, take notes, and run code smoothly. Here is an example of my set up:
...
## Listing
When you open a folder using the GUI on your computer, you can immediately see the files that are inside. In the terminal you will have to give it a command to see what is inside (ignore this con, the pros are coming soon). The command to "list" the contents of the **current directory** is:
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022/blob/main/bash/solutions/ex1.sh)


## Navigating Directories
Navigating through directories is like navigating through a city. There are streets that need to be followed in order to get to a specific house. In the real world, there are multiple ways to get to a single location like your house. But for this example, imagine there is only one set of directions to get to your house. That is what navigating your computer through a terminal is like (note: this is not true, but using short cuts in the terminal can be very tricky, so we are not going to talk about that). So imagine you are at Weiss Hall on Temple University's campus and you want to get to City Hall. What are the directions that you would need to take? Cecil B. Moore to Broad st. to City Hall. When you navigate through your computer's directory structure, you can't just say the final location of the file you are looking for, you need to give it a path, like how we just described going from Weiss Hall to City Hall. 
...
Here, we one had one step to go where we needed to go. For example, we are already on Broad St and the next stop is our destiation, City Hall. But what about when you are a few steps away from your destination?
...
#### Exercise 2.a
Navigate to the directory that your COG bootcamp materials are in using the terminal.You will probably have to give multiple directions in order to get to your destination. Navigate to each directory (streets in our City Hall example) and then arrive at your desired directory. 
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022/blob/main/bash/solutions/ex2a.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022/blob/main/bash/solutions/ex2b.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022/blob/main/bash/solutions/ex2c.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022/blob/main/bash/solutions/ex2d.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022/blob/main/bash/solutions/ex2e.sh)
...
#### Exercise 2.f
Copy over the subject data, in the "logs", from the "misc" workshop directory into the "subject_data" subdirectory in the "bash" directory.
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022/blob/main/bash/solutions/ex2f.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022/blob/main/bash/solutions/ex2g.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022/blob/main/bash/solutions/ex3.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022/blob/main/bash/solutions/ex4.sh)
...
### R
[See instructions on R day page](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2022//tree/main/R)


## Symbolic links
Symbolic links are shortcuts for files in the terminal. These are useful for when you want to reference something in another directory, instead of creating a duplicate of that directory. For example, you have some data you are analyzing and want to try something new with it. You could copy the current directory and then just replace files with your new analysis. Or, you could create a "master" directory for your data, then create subdirectories for each analysis. Inside each analysis subdirectory, you could create a symbolic link to the original data. 

The syntax to create a symbolic link is
```{eval=FALSE}
ln -s /source/file /path/to/where/the/shortcut/goes
```


## Reading files
Bash can be used to quickly see the contents of a text file. This can be done with `cat`.
```{eval=FALSE}
cat [filename]
```

Note that it's not only ".txt" files that can be read. Some text files have other extensions. For example, csv files are actually text files with commas separating the cells. Try reading the HCP dataset csv file using cat.


### Filtering 
As you can see, that was a lot of output and it was probably hard to make sense of any of it. Let's try filtering this data to find just a specific subject's data. We can do this by "piping" and then use `grep` to filter. Piping refers to running a command on the output of another command. The syntax to pipe is:
```{eval=FALSE}
command1 ... | command2
```
The pipe character above is the character you get from hitting shift and backslash (\) on your keyboard.

Let's try filtering the output of the HCP data for a single subject's data
```{eval=FALSE}
cat [filename] | grep [subjectID]
```


### Saving outputs to files
We can save the output of commands to files by using the `>` or `>>` characters. `>` saves the output to a brand new file, and overwrites any file with the name you provide it. `>>` appends your output to a file of a given name. If such a file doesn't exist, it is created. Try playing around with both commands.
```{eval=FALSE}
cat [filename] | grep [subjectID] > [output_file_name]
```


## Searching for files/filepaths
Directory structures can be really confusing. Things can get lost, or sometimes you just want to get the path for something that you know exists somewhere in the current directory. For example, I know there are solutions to all the exercises in this tutorial and that all of the files have a ".sh" extension. Try to use the `find` command to find all the solution files. Here's the find syntax:
```{eval=FALSE}
